...
/*
    @Test
    void testCalculateTotalSalesForWeek_IncludesOnlyThisWeek() {
        Date start = startOfWeek();

        // inside week (1 hour after start)
        Date insideWeek = new Date(start.getTime() + 60 * 60 * 1000);

        // outside week (1 hour before start)
        Date beforeWeek = new Date(start.getTime() - 60 * 60 * 1000);

        // near "now" but safely before endOfWeek (endOfWeek is new Date() inside method)
        Date nearNow = new Date(System.currentTimeMillis() - 2000);

        manager.addSalesRecord("A", 1, 10.0, insideWeek);
        manager.addSalesRecord("B", 1, 99.0, beforeWeek); // should NOT count
        manager.addSalesRecord("C", 1, 5.0, nearNow);
