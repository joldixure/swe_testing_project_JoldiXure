...
public class manageProfiles {
    ...
    public static void showManageProfilesDialog(ObservableList<Customer> selectedCustomers) {
        ...
        if (selectedCustomers.size() == 1) {
            Customer selectedCustomer = selectedCustomers.get(0);
        ...
        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == saveButtonType) {
                ...
                String history = purchaseHistoryField.getText();

                // Validation: contact must be at least 10 digits
                if (contact == null || contact.trim().length() < 10) {
                    Alert alert = new Alert(Alert.AlertType.ERROR);
                    alert.setTitle("Invalid Contact");
                    alert.setHeaderText("Contact number is invalid");
                    alert.setContentText("Contact number must be at least 10 digits.");
                    alert.showAndWait();
                    return null; // result yok -> kaydetme
                }


                return new Pair<>(name, contact);
            }
            return null;
        });
        ...
        dialog.showAndWait().ifPresent(profileData -> {
            ...
            for (Customer selectedCustomer : selectedCustomers) {
